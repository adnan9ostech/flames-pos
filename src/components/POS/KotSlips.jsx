...
/*
 * One kitchen slip — the station ticket for a single category of a round.
 *
 ...
 *   <KotSlips job={kotJob} />
 *
 * where job = { slip, meta } — `slip` is one element of
 * groupRoundByCategory()'s result, `meta` the round context shared by every
 * slip in the pass. The full sequencing contract lives atop src/lib/kotPrint.js.
 *
...
const KotSlips = ({ job }) => {
    ...
    const { slip, meta = {} } = job;
    const at = meta.at instanceof Date ? meta.at : new Date(meta.at || Date.now());
    ...
    return (
        ...
        <div id="kot-print-root" className={styles.slip} aria-hidden="true">
            ...
            <div className={styles.ticketType}>Kitchen Order</div>

            <div className={styles.meta}>
                <div className={styles.metaRow}>
                    <span>{meta.orderNumber ? `Order #${meta.orderNumber}` : 'Order'}</span>
                    <span>Round {meta.roundNo || 1}</span>
                </div>
                <div className={styles.metaRow}>
                    <span>
